...
Client.prototype.shellEx = function (id, command, cb) {
    ...
    this.shell(id, command, function (err, stream) {
        ...
        adb.util.readAll(stream, function (err, output) {
            ...
            ar.length--;
            for (let i = ar.length - 1; i > Math.max(-1, ar.length - 10); i--) {
...
Client.prototype.getIP = function (id, cb) {
    this.getProperties(id, (err, properties) => {
        let ip;
        if (!err && properties) {
        ...
        this.shellEx(id, 'ifconfig wlan0', (err, ar) => {
            if (err || !ar) {
                return cb && cb('');
            }
...
const adapter = soef.Adapter(main, onStateChange, onUnload, onMessage, adapterName);
...
const usedStateNames = {
    ...
    startApp: {
        n: 'startApp',
        val: '',
        common: { desc: 'start an application e.g.: com.netflix.ninja/.MainActivity' },
    },
    ...
    sendKeyCodeArray: {
        ...
        val: '',
        common: {
            desc: 'Can be an array of keys and delays. e.g.: 4000, DOWN, 100, DOWN, DOWN, LEFT, ENTER, 5000, LEFT',
        },
    },
...
FireTV.prototype.startClient = function (cb) {
    ...
    this.client.connect(this.id, 5555, (err, id) => {
        if (err || !id) {
            adapter.log.error(`can not connect to ${this.id} Error=${err.message}`);
            return;
        }
        this.client.id = id;
        this.client.getState(id, (err, state) => {
            adapter.log.debug(`Connected to ${this.id} id=${id}${!err && state ? ` state=${state}` : ''}`);
            this.updateState(state);
        });

        // this.client.getProperties(id, function(err, properties) {
        // });
        // this.client.getPackages(id, function(err, packages) {
        //     if (err || !packages) return;
        // });
        this.client.version((err, version) => {
            this.getAndroidVersion(androidVersion => {
                this.getAPILevel(apiLevel => {
                    soef.log(
                        '%s: ADB version: %s, Android Version: %s, API Level: %s',
                        this.id,
                        version,
                        androidVersion,
                        apiLevel,
...
FireTV.prototype.close = function () {
    if (this.client && this.client.id) {
        ...
        this.client.kill(function (err) {
            if (err) {
                adapter.log(`error killing adb server: ${err.message}`);
            }
        });
...
FireTV.prototype.handleCallback = function (err, stream, cb) {
    if (err || !stream) {
        ...
        if (err && err.message) {
            this.dev.setImmediately('error', err.message);
        }
        return cb && cb();
    }
    adb.util.readAll(stream, (err, output) => {
        if (!err && output) {
            const ar = output.toString().split(os.EOL);
            ar.length--;
            ...
            //     this.dev.setImmediately('result', line);
            // }
            if (ar.length < 10) {
                ar.forEach(line => {
                    //adapter.log.debug(line);
...
FireTV.prototype.shell = function (command, cb) {
    if (!this.client || !this.client.id) {
        return cb && cb('client.id not set');
    }
    this.client.shell(
        this.client.id,
        command,
...
FireTV.prototype.inputKeyevent = function (val) {
    if (~~val !== 0) {
        return this.shell(`input keyevent ${val}`);
    }
    ...
    const doIt = () => {
        if (i < ar.length && !this.stopKeyevents) {
            const v = ar[i++].trim();
            if ((number = ~~v)) {
                ...
                if (reInputKey.test(v)) {
                    ...
                    this.dev.setImmediately('result', 'callback');
                } else {
                    key = `input keyevent ${getKeyValue(v)}`;
                }
                //console.log('Sending: ' + key + ' i=' + i);
                this.shell(key, _lines => {
...
FireTV.prototype.frameBuffer = function (_val) {
    this.client.framebuffer(this.client.id, 'raw', function (err, stream) {
        if (err || !stream) {
            return;
        }
        adb.util.readAll(stream, function (_err, _output) {
            // ignore
        });
    });
};
...
function prepareDevices(cb) {
    ...
    g_client.listDevices((err, devices) => {
        ...
        devices.forEach(device => {
            if (!re.test(device.id)) {
                g_client.tcpip(device.id, (err, port) => {
                    if (err || !port) {
                        return cb && cb(err);
                    }
                    g_client.waitForDevice(device.id, (_err, _data) => {
                        g_client.getIP(device.id, ip => {
                            if (ip) {
                                g_client.connect(ip, port, (err, _data) => cb && cb(err));
                            }
                        });
                    });
                });
...
function main() {
    ...
    checkIP(() => startFireTVs(() => trackDevices()));
